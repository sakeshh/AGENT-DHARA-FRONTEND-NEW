...
function generateHtmlReport(data: any[]): string {
  const rows = data.map((r: any) => `
    <tr>
      ...
      <td>${r.totalColumns ?? '-'}</td>
      <td>${r.missingValues?.toLocaleString() ?? '-'}</td>
      <td>${r.duplicates?.toLocaleString() ?? '-'}</td>
      <td>${r.dataQualityScore ?? '-'}%</td>
    </tr>
  `).join('');
  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Data Assessment Report</title>
<style>body{font-family:system-ui;background:#ffffff;color:#0a0a0a;padding:2rem;}
table{border-collapse:collapse;width:100%;} th,td{border:1px solid #e5e7eb;padding:12px;text-align:left;}
th{background:#f4f4f5;color:#059669;} tr:nth-child(even){background:#fafafa;}
h1{color:#059669;margin-bottom:1.5rem;}</style></head>
<body><h1>Data Assessment Report</h1>
<table><tr><th>File</th><th>Rows</th><th>Columns</th><th>Missing</th><th>Duplicates</th><th>Quality</th></tr>
${rows}</table><p style="margin-top:2rem;color:#52525b;">Generated by Agent DHARA</p></body></html>`;
}
...
export default function DataPipelinePage() {
  ...
  const handleDownloadReport = () => {
    ...
    a.href = url;
    a.download = `data-assessment-report.${reportFormat === 'JSON' ? 'json' : 'html'}`;
    a.click();
              ...
              <div className="space-y-6">
                {!showReportView ? (
                  ...
                  <>
                    ...
                    <div className="options-scroll max-h-80 rounded-xl border border-black/10 bg-white/90 p-4 overflow-y-auto">
                      {reportFormat === 'JSON' ? (
